...
// The above code defines the reducer function.
...
const store = createStore(reducer);        // 1. Holding the application state right here. Here, the createStore function is used to create a Redux store.

console.log("Initial State", store.getState());    // 2.  The getState() method returns the current state held in the store.

const unsubscribe = store.subscribe(() =>
  console.log("Updated state", store.getState())     // 3. The subscribe() function is used to register a callback that will be invoked whenever the state in the store changes.
);
